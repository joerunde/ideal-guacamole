...
class MLFKTAdaptTransitionDifficultyModel:
    ...
    def __init__(self, X, P, intermediate_states, Dsigma, L1=False):
        ...
        for c in range(numprobs):
            ...
            self.params['Tf_' + str(c)] = parameter.Parameter(0,0,1, (lambda x: 1), (lambda x: self.bernoulli(.5) + 0*x))

            if L1:
                print "Using L1"
                self.params['D_' + str(c)] = parameter.Parameter(0, -3, 3, (lambda x, d_sig: laplace.pdf(x, 0, .5) + 0*d_sig),
                                                             (lambda x: np.random.normal(x, 0.15)))
                self.params['Tr_' + str(c)] = parameter.Parameter(0, -3, 3, (lambda x, d_sig: laplace.pdf(x, 0, .5) + 0*d_sig),
                                                             (lambda x: np.random.normal(x, 0.15)))
            else:
                print "Using adaptive L2"
                self.params['D_' + str(c)] = parameter.Parameter(0, -3, 3, (lambda x, d_sig: norm.pdf(x, 0, d_sig)),
                                                             (lambda x: np.random.normal(x, 0.15)))
                self.params['Tr_' + str(c)] = parameter.Parameter(0, -3, 3, (lambda x, d_sig: norm.pdf(x, 0, d_sig)),
                                                             (lambda x: np.random.normal(x, 0.15)))
    ...
    def make_emissions(self, diff, prob_num):
        ...
        g = expit(self.params['G_0'].get() - diff)
    ...
    def log_posterior(self, paramID, fake=False):
        ...
        for n in range(N):
            ...
            trans = self.make_transitions(self.params['Tr_' + str(int(Probs[n,0]))].get(), int(Probs[n,0]))
            trans_first = self.params['Tf_' + str(int(Probs[n,0]))].get()
            ...
            for t in range(1,T):
                ...
                trans = self.make_transitions(self.params['Tr_' + str(int(Probs[n,t]))].get(), int(Probs[n,t]))
                trans_first = self.params['Tf_' + str(int(Probs[n,t]))].get()
        ...
        if 'D_' in paramID or 'Tr_' in paramID:
