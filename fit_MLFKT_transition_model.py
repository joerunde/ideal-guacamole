...
print "usage: python fit_MLFKT_transition_model.py burnin iterations k(for 1/k test split) bkt(y/n) skills num_intermediate_states iterations L1/L2"
...
for it in range(iterz):
    ...
    if 'y' in sys.argv[4]:
        model = MLFKTTransitionModel(X, P, intermediate_states, 0, L1)
    else:
        model = MLFKTTransitionModel(X, P, intermediate_states, 0.1, L1)
    ...
    rmsefname = 'dump/RMSE_' + fname + "_trans_" + str(total_states) + "states_" + str(num_iterations) +"iter" + '.json'
    ...
    paramfname = 'dump/PARAMS_' + fname + "_trans_" + str(total_states) + "states_" + str(num_iterations) +"iter" + '.json'
