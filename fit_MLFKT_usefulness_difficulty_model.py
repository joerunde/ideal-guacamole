...
from Metropolis.mlfkt_usefulness_difficulty_model import MLFKTSUDModel
import sys, json, time, random, os, math
import numpy as np

print "usage: python fit_MLFKT_usefulness_difficulty_model.py burnin iterations k(for 1/k test split) bkt(y/n) skills num_intermediate_states iterations"
...
for it in range(iterz):
    ...
    if 'y' in sys.argv[4]:
        model = MLFKTSUDModel(X, P, S, intermediate_states, 0)
    else:
        model = MLFKTSUDModel(X, P, S, intermediate_states, 0.1)
    ...
    rmsefname = 'dump/RMSE_usefuldiff_' + fname + "_" + str(total_states) + "states_" + str(num_iterations) +"iter" + '.json'
    ...
    paramfname = 'dump/PARAMS_usefuldiff_' + fname + "_" + str(total_states) + "states_" + str(num_iterations) +"iter" + '.json'
