...
class MLFKTTransitionDifficultyGaussModel:
    ...
    def log_posterior(self, paramID, fake=False):
        ...
        for n in range(N):
            ...
            trans = self.make_transitions(self.params['D_' + str(int(Probs[n,0]))].get(), int(Probs[n,0]), pi[1])
            ...
            for t in range(1,T):
                ...
                belief = alpha[t-1,1] / np.sum(alpha[t-1,:])
                trans = self.make_transitions(self.params['D_' + str(int(Probs[n,t]))].get(), int(Probs[n,t]), belief)
