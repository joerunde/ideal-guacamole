...
from Metropolis.mlfkt_transition_difficulty_gaussian_model import MLFKTTransitionDifficultyGaussModel
import sys, json, time, random, os, math
import numpy as np

print "usage: python fit_MLFKT_transition_difficulty_gaussian_model.py burnin iterations k(for 1/k test split) bkt(y/n) skills num_intermediate_states iterations L1/L2 [transition first?]"
...
for it in range(iterz):
    ...
    if 'y' in sys.argv[4]:
        model = MLFKTTransitionDifficultyGaussModel(X, P, intermediate_states, 0, L1, trans_first)
    else:
        model = MLFKTTransitionDifficultyGaussModel(X, P, intermediate_states, 0.1, L1, trans_first)
    ...
    rmsefname = 'dump/RMSE_' + fname + "_transdiffgauss_" + str(total_states) + "states_" + str(num_iterations) +"iter" + '.json'
    ...
    paramfname = 'dump/PARAMS_' + fname + "_transdiffgauss_" + str(total_states) + "states_" + str(num_iterations) +"iter" + '.json'
